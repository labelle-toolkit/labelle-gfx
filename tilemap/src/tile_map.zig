...
/// The `parseTileset` counterpart of `parseAndFreeTilesetElement`.
///
/// Both entry points reach the same multi-`<image>` handling, and #336 and
/// #339 independently wrote the same free-before-dupe ordering off `main`,
/// so both need their own allocation-failure guard — a fix to one does not
/// exercise the other's caller.
fn parseAndFreeTileset(allocator: std.mem.Allocator, content: []const u8) !void {
    var pos: usize = "<tileset".len;
    const tileset = try TileMap.parseTileset(allocator, content, &pos);
    if (tileset.name.len > 0) allocator.free(tileset.name);
    if (tileset.image_source.len > 0) allocator.free(tileset.image_source);
}

test "parseTileset survives allocation failure on a second <image>" {
    const multi_image =
        \\<tileset name="multi" tilewidth="16" tileheight="16" columns="2" tilecount="2">
        \\ <image source="first.png" width="32" height="16"/>
        \\ <image source="second.png" width="32" height="16"/>
    ...
    {
        var pos: usize = "<tileset".len;
        const tileset = try TileMap.parseTileset(std.testing.allocator, multi_image, &pos);
        defer std.testing.allocator.free(tileset.name);
        defer std.testing.allocator.free(tileset.image_source);
        try std.testing.expectEqualStrings("second.png", tileset.image_source);
    }
